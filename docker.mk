...
run:
	docker run --rm -it $(foreach v,$(VOLUMES),-v $v) -w $(WORK) $(NAME) make run

ash: run/ash
bash: run/bash

run/%:
	docker run --rm -it $(foreach v,$(VOLUMES),-v $v) -w $(WORK) $(NAME) $(@F)

make/%:
	docker run --rm -it $(foreach v,$(VOLUMES),-v $v) -w $(WORK) $(NAME) make $(@F)
